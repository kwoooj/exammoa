...
test('SITE_ORIGIN 이 있으면 그 값을 쓴다', () => {
  const r = resolveOrigin({ SITE_ORIGIN: 'https://all.exammoa.workers.dev' });
  assert.equal(r.origin, 'https://all.exammoa.workers.dev');
...
test('끝 슬래시와 앞뒤 공백을 흡수한다', () => {
  ...
  assert.equal(resolveOrigin({ SITE_ORIGIN: '  https://all.exammoa.workers.dev/  ' }).origin,
    'https://all.exammoa.workers.dev');
});
...
test('https 가 아니면 거부한다 — localhost 만 예외', () => {
  assert.throws(() => resolveOrigin({ SITE_ORIGIN: 'http://all.exammoa.workers.dev' }), PrerenderError);
  assert.equal(resolveOrigin({ SITE_ORIGIN: 'http://localhost:4173' }).origin, 'http://localhost:4173');
});

test('주소로 읽히지 않으면 거부한다', () => {
  assert.throws(() => resolveOrigin({ SITE_ORIGIN: 'all.exammoa.workers.dev' }), PrerenderError);
});
...
test('배포 맥락에서 자리표시자로 떨어지지 않고 실패한다', () => {
  ...
  assert.throws(() => resolveOrigin({ WORKERS_CI: '1' }), PrerenderError);
...
test('프로덕션 robots 는 sitemap 을 가리키고 막지 않는다', () => {
  const txt = robotsTxt({ origin: 'https://all.exammoa.workers.dev', indexable: true });
  assert.ok(txt.includes('Sitemap: https://all.exammoa.workers.dev/sitemap.xml'));
  assert.ok(txt.includes('Allow: /'));
  assert.ok(!txt.includes('Disallow'));
});

test('미리보기 robots 는 전부 막는다', () => {
  const txt = robotsTxt({ origin: 'https://all.exammoa.workers.dev', indexable: false });
  assert.ok(txt.includes('Disallow: /'));
  // sitemap 을 내주면 막아 놓고 목록을 건네는 셈이다
  assert.ok(!txt.includes('Sitemap:'), txt);
});

test('프로덕션 브랜치의 Workers Build 만 색인한다', () => {
  assert.equal(isIndexable({ WORKERS_CI: '1', WORKERS_CI_BRANCH: 'mvp' }), true);
  assert.equal(isIndexable({ WORKERS_CI: '1', WORKERS_CI_BRANCH: 'fix/58-workers-deploy' }), false);
  assert.equal(isIndexable({ WORKERS_CI: '1' }), false);

// 한국금융연수원 금융·은행권 1차 자격 4종.
//
// 공개 시험일정 화면은 날짜 목록을 JSON으로 내려준다. 화면에 명시된 공통 접수시간
// (시작 10:00, 마감 20:00)과 JSON의 시험시간을 함께 보존한다. 일정이 서로 다른 네
...
export function parse(raw, { year }) {
  ...
  const targetRows = rows.filter(row => TARGETS[String(row.I_QLFN)] && Number(row.D_YY) === year);
  const foundCodes = new Set(targetRows.map(row => String(row.I_QLFN)));
  const structureOk = Boolean(clockMatch) && Object.keys(TARGETS).every(code => foundCodes.has(code));
  const failures = [];
  const sessions = [];
  ...
  return {
    sessions,
    diagnostics: { rows: targetRows.length, parsed: sessions.length, headerMatch: structureOk, failures },
  };
}

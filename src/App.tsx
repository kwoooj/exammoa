...
cp -r build public/data     # 또는 build 를 public/data 로 복사`}</pre>
  ...
  return (
    <main>
      ...
      <p>
        타임라인의 한 행은 종목이 아니라 <strong>시행그룹</strong>입니다. 종목 {data.meta.examCount}
        개의 일정이 실제로는 {data.meta.groupCount}가지뿐이라 (접기 전 회차{' '}
        {data.meta.sessionsBeforeFold}건 → {data.meta.sessionCount}건), 종목별로 행을 그리면 같은
        막대가 여러 줄 반복됩니다.
      </p>

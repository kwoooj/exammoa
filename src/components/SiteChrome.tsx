 ...
 * 헤더와 푸터. 화면정의 §2.1 · §2.3.
 *
 * 통합 검색과 관심 시험 캘린더 배지는 S-02 를 만드는 PR 에서 붙인다. 지금은 라우트를
...
export function SiteHeader() {
  ...
  const calendarHref = data && favorites.length > 0
    ? `${ROUTE_PATHS.calendar}?view=favorites`
    ...
    <header className="hdr">
      <div className="hdr__inner">
        ...
        <nav className="hdr__nav" aria-label="주요 메뉴">
          <Link to={calendarHref} className="hdr__calendar">
            <CalendarBlank size={19} aria-hidden="true" />
            <span>시험 캘린더</span>
            {favorites.length > 0 && <span className="hdr__badge">{favorites.length}</span>}
          </Link>
        </nav>
      </div>
    </header>
  ...
  return (
    <footer className="footer">
      <div className="footer__inner">
        <div>
          <Link to={ROUTE_PATHS.home} className="footer__brand">시험모아</Link>
          <p className="small muted">여러 기관의 시험 일정을 한곳에 모아 보여주는 비공식 정보 서비스입니다.</p>
        </div>
        ...
        */}
        ...
        시험 {data.meta.examCount}개 · 시행그룹 {data.meta.groupCount}개
          ...
          <p className="small footer__links">
      ...
      */}
      ...
      <details className="sources footer__sources">
